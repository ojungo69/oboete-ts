...
empty, or foreign id is rejected as `unusable_output` (one retry). The request names its events
`e1..eN` and its nearby records `m1..mN` instead of their stored ids, in a table that belongs to
that request alone (only `events[].id` and `nearby[].id`: `checkpoint_context.id`, fragment fields
and any id inside text are sent as they are), and the answer's `source_event_ids`, checkpoint sources and nearby
