...
`docs/evidence/memory-core-2026-09/resource-sweep.md`; the run is 2026-09-21 against `7ca83db7` on
...
least 20 seconds (24.0 s and 26.7 s measured, the rest being the session-end hooks and the wait for a batch to
...
own stop path runs `wal_checkpoint(TRUNCATE)`; and peak `VmHWM` under 150 MiB (108.20 MiB and
100.16 MiB) across every process of the run — the worker from the samples, and all 245 children
from the kernel's figure at exit, which is what carries phase A's own 1,143 hooks into the bound.
Injection p99 (284.6 ms), two session-start packs without `summary_pending` and two phase B hooks
