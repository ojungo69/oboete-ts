...
`docs/evidence/memory-core-2026-09/resource-sweep.md`; the run is 2026-09-21 against `f53bc0f6` on
...
(108.23 MiB and 100.67 MiB) across every process of the run — the worker from the samples, and all
...
Injection p99 (296.9 ms) and two session-start packs without `summary_pending` are reported rather
