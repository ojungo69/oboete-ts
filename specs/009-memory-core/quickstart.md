...
`docs/evidence/memory-core-2026-09/resource-sweep.md`; the run is 2026-09-21 against `99674c54` on
...
least 20 seconds (24.3 s measured, the rest being the session-end hooks and the wait for a batch to
...
(107.46 MiB and 100.86 MiB) across every process of the run — the worker from the samples, and all
...
Injection p99 (300.4 ms) and two session-start packs without `summary_pending` are reported rather
